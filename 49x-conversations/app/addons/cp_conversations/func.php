...
// ... existing code ...
    if (!empty($message_id)) {
        ...
        // Отправка ответа в ЦИАН, если диалог привязан
        if (!empty($message_data['conversation_id'])) {
            $cian_chat_id = db_get_field("SELECT cian_chat_id FROM ?:cp_conversations_cian_map WHERE conversation_id = ?i", $message_data['conversation_id']);
            
            // Проверяем, что сообщение пишет продавец/админ (не 0) и это чат ЦИАН
            if ($cian_chat_id && !empty($message_data['user_id'])) {
                require_once Registry::get('config.dir.addons') . 'cp_conversations/func.cian.php';
                fn_cp_conversations_cian_send_message($cian_chat_id, $message_data['message']);
            }
        }
    }
