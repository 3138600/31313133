...
function fn_cp_similar_products_get_products_pre(&$params, &$items_per_page, &$lang_code)
{
    if (!empty($params['cp_similar_by_feature']) || !empty($params['cp_similar_by_tags'])) {
        ...
        // ИСПРАВЛЕНИЕ SQL ОШИБКИ 1054: В CS-Cart 4 поля price и category_id 
        // находятся в связанных таблицах (product_prices и products_categories).
        $product_status = db_get_field("SELECT status FROM ?:products WHERE product_id = ?i", $product_id);
        
        // Проверяем, существует ли товар и активен ли он ('A' - Active, 'H' - Hidden)
        ...
        if (!empty($params['cp_similar_by_feature']) && !empty($params['feature_ids'])) {
            ...
            if (!empty($current_variants)) {
                ...
                if (empty($feature_weights)) {
                    // ОПТИМИЗАЦИЯ 1: Strict AND через SQL GROUP BY
                    ...
                    // ОПТИМИЗАЦИЯ 2: Взвешенный поиск
        ...
        elseif (!empty($params['cp_similar_by_tags'])) {
            // ИСПРАВЛЕНИЕ: Проверка активности модуля Теги для избежания SQL ошибок
        ...
        // ИСПРАВЛЕНИЕ: НАСТРОЙКИ НАЛИЧИЯ (Без обрезки предзаказов для 4.9.2)
