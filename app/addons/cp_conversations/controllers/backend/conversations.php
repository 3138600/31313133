...
//
    ...
    if(!empty($_REQUEST['conversation_id'])) {
        ...
        if(!empty($conversation)) {
            ...
            //if this conversation is linked to a CIAN chat, mark it read on CIAN side as well
            $cian_chat_id = db_get_field("SELECT cian_chat_id FROM ?:cp_conversations_cian_map WHERE conversation_id = ?i", $conversation['conversation_id']);
            if (!empty($cian_chat_id) && function_exists('fn_cp_conversations_cian_read_chat')) {
                fn_cp_conversations_cian_read_chat($cian_chat_id);
            }
